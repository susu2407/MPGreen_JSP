...
-- Table `cou_req`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cou_req` ;

CREATE TABLE IF NOT EXISTS `cou_req` (
...
USE `green2`$$
CREATE DEFINER = CURRENT_USER TRIGGER `green2`.`COURSE_BEFORE_INSERT` BEFORE INSERT ON `COURSE` FOR EACH ROW
BEGIN
    -- 필요한 변수들을 선언
    DECLARE current_year VARCHAR(4);
